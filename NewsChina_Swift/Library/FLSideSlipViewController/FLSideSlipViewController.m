...
#define defaultLeftDistance 200
...
static const CGFloat min_Alpha = 1.0f;/**< 背景最小的透明度*/
//static const CGFloat defaultLeftDistance = 250.0f*DSScaleNum;/**< 默认离左边的距离*/
static const CGFloat defaultScale = 0.80;/**< 默认缩小范围*/
...
- (instancetype)initWithRootViewController:(UIViewController *)viewController withLeftViewController:(UIViewController *)leftViewController {
    if ((self = [super init])) {
        self.rootViewController = viewController;
...
//    [g_backGroundImage setImage:[UIImage imageNamed:@"backGroundImage"]];
    g_backGroundImage.backgroundColor = [UIColor whiteColor];
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    ...
    if (_fs_rootViewController.childViewControllers.count == 1 && !_canSlideInPush) {
        return YES;
    }
    return NO;
}
...
-(void)pan:(UIPanGestureRecognizer *)gesture{
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        [self showShadow:YES];
        [_fs_rootViewController.view setNeedsDisplay];
        g_currentTranslateX = _fs_rootViewController.view.frame.origin.x;
        ...
        CGPoint translation = [gesture translationInView:_fs_rootViewController.view];//滑动距离
        CGFloat transX = translation.x;
        if (_fs_rootViewController.view.frame.origin.x >30 &&_fs_rootViewController.view.frame.origin.x < 35) {
            [self reloadLeftTableViews];
        }
...
//            _fs_rootViewController.view.transform = transIdentify;
            return;
        }
        switch (_animationType) {
            case AnimationTransition: {
                CGAffineTransform transT = CGAffineTransformMakeTranslation(transX, 0);
                _fs_rootViewController.view.transform = transT;
                break;
            }
            case AnimationTransitionAndScale: {
                ...
                _fs_rootViewController.view.transform = conT;
                break;
            }
            case AnimationTransitionAndScaleAndIncline: {
              
                if (transX < defaultLeftDistance) {    //为了使滑到最大距离之后不执行缩放以及滑动操作
                    CATransform3D transform = CATransform3DIdentity;
                    transform.m34 = 0.5/ -600;//透视效果，要操作的这个对象要有旋转的角度，否则没有效果。正直/负值都有意义
                    transform.m11 = sca;//x轴缩放
                    //滑动的时候这里配合我们的设计效果出来不是很好，这里做了一点小改动
                    if (sca+0.1 >= 0.996992) {
                        transform.m22 = 0.996000;
                    }
                    else {
                      transform.m22 = sca+0.1;//y轴缩放
                    }
                    ...
                    _fs_rootViewController.view.layer.transform = transform;
                    _fs_rootViewController.view.layer.shouldRasterize = YES;//抗锯齿
                   [_fs_rootViewController.view.layer setRasterizationScale:[[UIScreen mainScreen] scale]];
                }
                else {
                    return;
                }
        ...
        if (velocity.x > 0 && !g_menuFlags.showingRightView && g_menuFlags.canShowLeft) {
            [self setStatusBarHiddenWithIsHidden:YES];
            if (!g_menuFlags.showingLeftView) {
                g_menuFlags.showingLeftView = YES;
                CGRect leftFrame = self.view.bounds;
                CGAffineTransform oriT = CGAffineTransformIdentity;
            ...
            [_fs_rootViewController.view addSubview:g_overView];
        ...
        if (velocity.x < 0 && !g_menuFlags.showingLeftView && g_menuFlags.canShowRight) {
            ...
            [self setStatusBarHiddenWithIsHidden:NO];
            _fs_rootViewController.view.layer.shouldRasterize = NO;//取消抗锯齿
            CGAffineTransform oriT = CGAffineTransformIdentity;
            CGAffineTransform transRight = CGAffineTransformMakeTranslation(-_leftDistance, 0);
            CGAffineTransform transScale = CGAffineTransformMakeScale(_scaleSize, _scaleSize);
            CGAffineTransform transConcat = CGAffineTransformConcat(transRight, transScale);
            ...
            [UIView animateWithDuration:.3 animations:^{
                _fs_rootViewController.view.transform = oriT;
...
-(void)setNewRootViewController:(UIViewController *)viewController animation:(BOOL)animation{
    ...
    if (g_menuFlags.showingLeftView) {
        ...
        CGRect rightFrame = _fs_rootViewController.view.frame;
        rightFrame.origin.x = self.view.frame.size.width;
        [UIView animateWithDuration:.1 animations:^{
            _fs_rootViewController.view.transform = transConcat;
        }completion:^(BOOL success){
...
////    [self setRootViewController:[[UINavigationController alloc] initWithRootViewController:viewController]];
    TTBaseViewController *baseVC = (TTBaseViewController *)viewController;
    if (baseVC.isPush) {  //为了不出现重叠的bug
        _canSlideInPush = NO;
        [(UINavigationController *)_fs_rootViewController pushViewController:viewController animated:animation];
    } else {
        _canSlideInPush = YES;
    }
      [self showRootViewController:YES];
...
-(CATransform3D)getCATransform3D{
    ...
    transform.m22 = transcale+0.1;//y轴缩放
...
- (void)showShadow:(BOOL)val {
    if (!_fs_rootViewController) return;
    
    _fs_rootViewController.view.layer.shadowOpacity = val ? 0.8f : 0.0f;
    if (val) {
        _fs_rootViewController.view.layer.cornerRadius = 4.0f;
        _fs_rootViewController.view.layer.shadowOffset = CGSizeZero;
        _fs_rootViewController.view.layer.shadowRadius = 4.0f;
        _fs_rootViewController.view.layer.shadowPath = [UIBezierPath bezierPathWithRect:self.view.bounds].CGPath;
    }
...
-(void)setNavigationBackButton{
    if (_fs_rootViewController) {
        
        UIViewController *topController = nil;
        if ([_fs_rootViewController isKindOfClass:[UINavigationController class]]) {
            
            UINavigationController *navController = (UINavigationController*)_fs_rootViewController;
            if ([[navController viewControllers] count] > 0) {
                topController = [[navController viewControllers] objectAtIndex:0];
            }
            
        } else if ([_fs_rootViewController isKindOfClass:[UITabBarController class]]) {
            
            UITabBarController *tabController = (UITabBarController*)_fs_rootViewController;
            topController = [tabController selectedViewController];
            
        } else {
            
            topController = _fs_rootViewController;
...
- (void)setRootViewController:(UIViewController *)rootViewController {
    UIViewController *tempRoot = _fs_rootViewController;
    _fs_rootViewController = rootViewController;
    _fs_rootViewController.view.layer.zPosition = 100.0f;//设置_rootviewcontroller画在界面上的层级
    _fs_rootViewController.view.layer.shouldRasterize = NO;//取消抗锯齿
    if (_fs_rootViewController) {
        if (tempRoot) {
        ...
        UIView *view = _fs_rootViewController.view;
        ...
        if (_fs_rootViewController.view) {
            [_fs_rootViewController.view addGestureRecognizer:g_tap];
        }
        [g_tap setEnabled:NO];
    } else {
        if (tempRoot) {
...
//刷新tableview动画
- (void)reloadLeftTableViews {
    if ([_leftViewController isKindOfClass:[TTMenuViewController class]]) {
        TTMenuViewController *leftVC = (TTMenuViewController *)_leftViewController;
//        leftVC.firstSwipe = NO;
//        leftVC.isSetting = NO;
        [leftVC.mTableView reloadData];
    }
}


#pragma mark --是否显示状态栏
- (void)setStatusBarHiddenWithIsHidden:(BOOL)ishidden {
